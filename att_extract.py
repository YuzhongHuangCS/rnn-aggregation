...
if True:# or not os.path.exists('cache.ckpt'):
	...
	ifp_test = [2565, 2943, 2907, 3069, 2997, 1145, 3114]
	print(ifp_train)
	print(ifp_test)
...
atts = tf.TensorArray(dtype=tf.float32, size=n_forecast_placeholder, element_shape=(max_steps, max_steps))

def compute(i, out, out_att):
	...
	return i+1, out.write(i, weighted_state), out_att.write(i, tf.matmul(state_series[index], tf.transpose(state_series[index])))

_, out1, out2 = tf.while_loop(
	lambda a, b, c: a < n_forecast_placeholder,
	compute,
	(0, output, atts)
)

att_state = out1.stack()
att_matrix = out2.stack()
...
with tf.Session() as sess:
	...
	test_loss, test_pred, att_need = sess.run(
		[loss_weighted, prob, att_matrix],
			feed_dict={
	...
	def show_plot(ifp):
		c_index = index_map_test[ifp][-1]
		c_len = len(db[ifp])

		m = sklearn.utils.extmath.softmax(att_need[c_index, :c_len, :c_len])
