...
vs = np.asarray(list(db_emb.values()))
titles = list(db_title.values())
X_emb = TSNE(n_components=2, random_state=1234).fit_transform(vs)

#import mpld3
#plt.figure(figsize=(200,200))
plt.scatter(X_emb[:, 0], X_emb[:, 1])
for i in range(len(vs)):
    plt.annotate(titles[i], (X_emb[i, 0], X_emb[i, 1]))
plt.show()
pdb.set_trace()
#mpld3.show()
