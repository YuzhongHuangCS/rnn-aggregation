...
for try_feature in ('n_forecasts', 'n_forecasts_b', 'n_forecasts_c', 'n_forecasts_d', 'n_forecasts_sage', 'n_forecasts_te', 'ordinal', 'p_updates', 'x_acf1', 'x_acf10', 'diff1_acf1', 'diff1_acf10', 'diff2_acf1', 'diff2_acf10', 'seas_acf1', 'ARCH.LM', 'crossing_points', 'entropy', 'flat_spots', 'arch_acf', 'garch_acf', 'arch_r2', 'garch_r2', 'alpha', 'beta', 'hurst', 'lumpiness', 'nonlinearity', 'x_pacf5', 'diff1x_pacf5', 'diff2x_pacf5', 'seas_pacf', 'nperiods', 'seasonal_period', 'trend', 'spike', 'linearity', 'curvature', 'e_acf1', 'e_acf10', 'seasonal_strength', 'peak', 'trough', 'hw_alpha', 'hw_beta', 'hw_gamma', 'unitroot_kpss', 'unitroot_pp', 'Health/Disease', 'Macroeconomics/Finance', 'Natural Sciences/Climate', 'Other', 'Politics/Intl Relations', 'Technology', 'entropy_b', 'entropy_c', 'entropy_d', 'entropy_human', 'entropy_sage', 'entropy_te', 'stage', 'variance_b', 'variance_c', 'variance_d', 'variance_human', 'variance_sage', 'variance_te', 'stability', 'series_length', 'skew', 'ratio'):
	output_filename = 'results/{}.txt'.format(try_feature.replace('/', '_'))
	...
	try_drop_list = drop_list + [try_feature]
	...
	for train_index, test_index in kf.split(ifp_all):
		...
		train_X = train_df.drop(columns=['ifp_id', 'date', 'answer', 'ordered', 'num_options']).drop(columns=try_drop_list)
		...
		test_X = test_df.drop(columns=['ifp_id', 'date', 'answer', 'ordered', 'num_options']).drop(columns=try_drop_list).values
	...
	fout.write(str(np.mean(splits, axis=0)) + '\n')
	fout.write(str(splits) + '\n')
	fout.close()
	print('OK')
