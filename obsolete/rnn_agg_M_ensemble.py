...
def M2_aggregation(train_input, test_input, db_dates,db_answer):
	...
	u = math.sqrt(6/(n_feature + 1))
	W = torch.FloatTensor(n_feature, 1).uniform_(-u, u)
	...
	for z in range(100):
		...
		scores = {}
		for ifp in ifps:
			scores[ifp] = torch.mean(torch.stack(results[ifp]))

		mmdb = torch.mean(torch.stack(list(scores.values())))
		print('Epoch: {}, alpha: {}, mmdb: {}'.format(z, alpha, mmdb))
