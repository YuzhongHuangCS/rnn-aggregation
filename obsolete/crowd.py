...
if True:# or not os.path.exists('cache.ckpt'):
	...
	if feature_used in human_feature_list and feature_used in hf_cols:
	...
	if feature_used in ts_feature_list and feature_used in tf_cols:
...
with tf.Session() as sess:
	...
	for ifp in ifp_test:
		...
		for p in individual_forecasts:
			local_brier = brier(p[4:9][:p[3]], answer, is_ordered)
			local_seconds = (p[0]-start_date).total_seconds()
			local_progress = int(np.around(100.0 * local_seconds / total_seconds))
			if local_progress > 99:
				local_progress = 99
			trend_ary[local_progress].append(local_brier)
		...
		for i, s in enumerate(scores):
			...
			if local_progress > 99:
				local_progress = 99
			trend_rnn_ary[local_progress].append(s)

	'''
	trend_sum = [np.mean(x) for x in trend_ary]
	trend_rnn_sum = [np.mean(x) for x in trend_rnn_ary]

	trend_sum_blur = gaussian_filter(trend_sum, sigma=7)
	trend_rnn_sum_blur = gaussian_filter(trend_rnn_sum, sigma=7)
	...
	'''
	with open('plot_data/crowd_trend_db_{}.pickle'.format(fold_index), 'wb') as fout:
		pickle.dump(trend_ary, fout, pickle.HIGHEST_PROTOCOL)

	with open('plot_data/rnn_trend_db_{}.pickle'.format(fold_index), 'wb') as fout:
		pickle.dump(trend_rnn_ary, fout, pickle.HIGHEST_PROTOCOL)
	...
	for ifp in ifp_test:
		...
		individual_forecasts = [x for x in db[ifp] if isinstance(x[1], int) or x[1] == 'Auto ARIMA']
		answer, is_ordered = db_answer[ifp]
	...
	#np.save(save_path.replace('.ckpt', '.npy'), test_pred)
print('OK')
