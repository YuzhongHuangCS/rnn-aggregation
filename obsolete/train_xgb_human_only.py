...
for train_index, test_index in kf.split(ifp_all):
	...
	train_X = train_df.drop(columns=['ifp_id', 'date', 'answer', 'ordered', 'num_options']).drop(columns=drop_list).drop(columns=drop_model_list)
	...
	test_X = test_df.drop(columns=['ifp_id', 'date', 'answer', 'ordered', 'num_options']).drop(columns=drop_list).drop(columns=drop_model_list).values
	...
	bst.save_model('human_only_{}.model'.format(counter))
	bst.dump_model('human_only_{}.txt'.format(counter))
