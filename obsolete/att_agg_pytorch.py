...
if True:# or not os.path.exists('cache.ckpt'):
	...
	max_steps = max([len(v) for k, v in db.items() if k in ifp_train])
...
id_train = torch.from_numpy(id_train).long()
gather_index_train = torch.from_numpy(gather_index_train).long()
for t in range(100):
	prob = model(input_train, id_train, gather_index_train)
