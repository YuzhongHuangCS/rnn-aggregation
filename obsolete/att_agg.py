...
if not os.path.exists('cache.ckpt'):
	...
	input_train = np.zeros((n_forecast_train, max_steps, 5))
	...
	input_test = np.zeros((n_forecast_test, max_steps, 5))
...
l1_filter = tf.get_variable('l1_w', shape=(1, 5+N_EMB_DIM, N_RNN_DIM))
l1_output = tf.nn.conv1d(combined_input, l1_filter, stride=1, padding='VALID')
#pdb.set_trace()
Att_W = tf.get_variable('Att_W', shape=(1, N_RNN_DIM, 1))
att = tf.reshape(tf.nn.conv1d(l1_output, Att_W, stride=1, padding='VALID'), (-1, max_steps))

#pdb.set_trace()
att_softmax = tf.nn.softmax(tf.math.add(att, seq_length_placeholder))
att_softmax_stack = tf.stack([att_softmax, att_softmax, att_softmax, att_softmax, att_softmax], axis=2)

prediction = tf.reduce_sum(att_softmax_stack * input_placeholder, axis=1)
prob = prediction
