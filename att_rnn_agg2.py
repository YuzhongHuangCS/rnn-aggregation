...
if True or not os.path.exists('cache.ckpt'):
	...
	kf = sklearn.model_selection.KFold(shuffle=True, n_splits=100, random_state=2019)
	folds = [[all_ifp[f[0]], all_ifp[f[1]]] for f in kf.split(all_ifp)]
	fold_index = fold_index_used

	ifp_train = folds[fold_index][1]
	ifp_test = folds[fold_index][1]
	print('ifp_train', ifp_train)
	print('ifp_test', ifp_test)
	...
	seq_length_train = np.full((n_forecast_train, max_steps, max_steps), -1e32)
	...
	for index, ifp in enumerate(ifp_train):
		...
		for i, forecast_date in enumerate(forecast_dates):
			...
			seq_length_train[forecast_index+i, :this_index, :this_index] = 0
	...
	seq_length_test = np.full((n_forecast_test, max_steps, max_steps), -1e32)
	...
	for index, ifp in enumerate(ifp_test):
		...
		for i, forecast_date in enumerate(forecast_dates):
			...
			seq_length_test[forecast_index+i, :this_index, :this_index] = 0
...
seq_length_placeholder = tf.placeholder(tf.float32, [None, max_steps, max_steps])
...
output = tf.matmul(q_output, tf.transpose(k_output, [0, 2, 1]))
output /= N_K_DIM ** 0.5

#pdb.set_trace()
output = tf.nn.softmax(tf.math.add(seq_length_placeholder, output))

#prediction = tf.reduce_sum(att_softmax_stack * input_placeholder[:, :, :5], axis=1)
weighted_state = tf.reduce_sum(tf.matmul(output, v_output), axis=1)
