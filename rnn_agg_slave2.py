...
if True:# or not os.path.exists('cache.ckpt'):
	...
	n_train_all = len(folds[fold_index][0])
	n_train = int(n_train_all * 0.8)
	n_valid = n_train_all-n_train

	ifp_train_all = folds[fold_index][0]
	ifp_train = folds[fold_index][0][:n_train]
	ifp_valid = folds[fold_index][0][n_train:]
	...
	### TRAIN ALL data
	n_forecast_train_all = sum([len(v) for k, v in db_dates.items() if k in ifp_train_all])

	input_train_all = np.zeros((n_train_all, max_steps, 5 + n_feature))
	id_train_all = np.zeros((n_train_all, max_steps), dtype=int)
	target_train_all = np.zeros((n_forecast_train_all, 5))
	answer_train_all = np.zeros(n_forecast_train_all, dtype=int)
	is_ordered_train_all = np.zeros(n_forecast_train_all, dtype=bool)
	is_4_train_all = np.zeros(n_forecast_train_all, dtype=bool)
	is_3_train_all = np.zeros(n_forecast_train_all, dtype=bool)
	weight_train_all = np.zeros(n_forecast_train_all)
	seq_length_train_all = np.zeros(n_train_all, dtype=int)
	gather_index_train_all = np.zeros((n_forecast_train_all, 2), dtype=int)
	num_option_ary_train_all = np.zeros(n_forecast_train_all, dtype=int)
	num_option_mask_train_all = np.full((n_forecast_train_all, 5), -1e32)
	index_map_train_all = {}

	forecast_index = 0
	for index, ifp in enumerate(ifp_train_all):
		forecasts = db[ifp]

		for i, forecast in enumerate(forecasts):
			input_train_all[index, i] = forecast[4:]
			forecaster_id = id2index.get(forecast[1], 1)
			id_train_all[index, i] = forecaster_id
		...
		answer, is_ordered = db_answer[ifp]
		target_train_all[forecast_index:forecast_index+n_forecasts, answer] = 1
		answer_train_all[forecast_index:forecast_index+n_forecasts] = answer
		is_ordered_train_all[forecast_index:forecast_index+n_forecasts] = is_ordered
		weight_train_all[forecast_index:forecast_index+n_forecasts] = ((1.0 / n_forecasts) / n_train_all) * n_forecast_train_all
		seq_length_train_all[index] = len(forecasts)
		...
		for i, forecast_date in enumerate(forecast_dates):
			...
			if this_index == 0:
				this_index = 1
			gather_index_train_all[forecast_index+i, :] = [index, this_index-1]

		num_options = forecasts[0][3]
		if num_options == 4:
			is_4_train_all[forecast_index:forecast_index+n_forecasts] = True
		else:
			if num_options == 3:
				is_3_train_all[forecast_index:forecast_index+n_forecasts] = True

		num_option_ary_train_all[forecast_index:forecast_index+n_forecasts] = num_options
		num_option_mask_train_all[forecast_index:forecast_index+n_forecasts, :num_options] = 0

		index_map_train_all[ifp] = list(range(forecast_index, forecast_index+n_forecasts))
		forecast_index += n_forecasts

	input_train_all[np.isnan(input_train_all)] = 0
	### TEST data
...
with tf.Session() as sess:
	...
	train_all_zero_state = np.zeros((n_train_all, N_RNN_DIM), dtype=np.float32)
	...
	train_all_loss, train_all_pred = sess.run(
		[loss_weighted, prob],
			feed_dict={
				input_placeholder: input_train_all,
				id_placeholder: id_train_all,
				target_placeholder: target_train_all,
				is_ordered_placeholder: is_ordered_train_all,
				is_4_placeholder: is_4_train_all,
				is_3_placeholder: is_3_train_all,
				weight_placeholder: weight_train_all,
				seq_length_placeholder: seq_length_train_all,
				gather_index_placeholder: gather_index_train_all,
				num_option_mask_placeholder: num_option_mask_train_all,
				zero_state: train_all_zero_state,
				is_training: True
			}
	)

	for i in range(100):
		if train_all_loss < smallest_train_loss:
			break

		train_all_loss, train_all_pred, _train_step = sess.run(
			[loss_weighted, prob, train_op],
				feed_dict={
					input_placeholder: input_train_all,
					id_placeholder: id_train_all,
