...
qk_output = tf.matmul(q_output, tf.transpose(k_output, [0, 2, 1])) / (N_K_DIM ** 0.5)
#pdb.set_trace()
#output = torch.empty((gather_t.shape[0], N_V_DIM))
...
def compute(i, out):
	...
	att = tf.nn.softmax(qk_output[index, :seq_length, :seq_length])
	v_local = v_output[index, :seq_length, :]
	weighted_state = tf.reduce_sum(tf.matmul(att, v_local), axis=0)
