...
for fold_index in range(5):
	...
	for i, ifp in enumerate(ifp_valid):
		...
		#scaler = StandardScaler()
		#briers = scaler.fit_transform(np.asarray(briers).reshape(-1, 1)).squeeze().tolist()
...
xx, yy, zz = kde2D(np.asarray(new_brier_ary), np.asarray(new_att_ary), 2.0)
plt.pcolormesh(xx, yy, zz)
plt.scatter(brier_ary, att_ary, s=2, facecolor='white')

#plt.scatter(new_brier_ary, new_att_ary)
'''
print(len(brier_ary), len(att_ary))
pairs = list(zip(brier_ary, att_ary))
pairs_left = np.asarray([x for x in pairs if x[0] <= 1e-8])
pairs_right = np.asarray([x for x in pairs if x[0] > 1e-8])
pdb.set_trace()

sns.regplot(np.asarray(new_brier_ary, dtype=np.float32), np.asarray(new_att_ary, dtype=np.float32))
plt.ylabel('Attention Score')
plt.xlabel('Normalized Brier')
plt.show()
pdb.set_trace()
print('Leaving')
