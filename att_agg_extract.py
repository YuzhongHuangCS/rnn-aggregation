...
#upper_triangle = tf.linalg.band_part(tf.constant(-3.0, shape=(max_steps, max_steps)), 0, -1)
...
att_scores = tf.nn.softmax(tf.linalg.band_part(tf.matmul(q_output, tf.transpose(k_output, [0, 2, 1])) / (N_K_DIM ** 0.5), -1, 0) + seq_length_mask_placeholder)
...
att_scores2 = tf.nn.softmax(tf.linalg.band_part(tf.matmul(q_output2, tf.transpose(k_output2, [0, 2, 1])) / (N_K_DIM ** 0.5), -1, 0) + seq_length_mask_placeholder)
...
with tf.Session() as sess:
	...
	db_brier_valid = {}
	db_rank_valid = []
	db_prediction_valid = {'score':[], 'true':[]}
	for ifp in ifp_valid:
		index = index_map_valid[ifp]
		scores = valid_briers[index]
		score = np.mean(scores)
		db_brier_valid[ifp] = score
		...
		for s in scores:
			...
			db_rank_valid.append(rank)

		preds = valid_pred[index]
		num_options = db[ifp][0][3]
		for pred in preds:
			db_prediction_valid['score'] += pred.tolist()[:num_options]
			true_pred = list(np.zeros(num_options))
			true_pred[answer] = 1
	...
	with open('plot_data/transformer_predictions_db_{}.pickle'.format(fold_index), 'wb') as fout:
